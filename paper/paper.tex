...
We use six metrics. Makespan is $M_T = \max(\mathrm{completion\_time}) - \min(\mathrm{arrival\_time})$. Load balance is $M_L$, the standard deviation of per-node CPU utilization. Resilience is $M_R = \mathrm{completed\_after\_failure} / \mathrm{orphaned}$. Efficiency is $M_E = (\mathrm{energy} + \mathrm{transmission}) / \mathrm{completed}$. Queue instability is $M_Q$, the time-averaged saturation above 0.85. Decision stability is $M_S = \mathrm{migrations} / \mathrm{completed}$. The radiation model \cite{m2-radiation} and PSPLIB integration \cite{m3-psplib} provide stochastic failure realism and dependency-structure realism, respectively.
